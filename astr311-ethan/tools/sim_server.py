...
Also serves the legacy 3-body / Pluto interactive simulations.
...
# ---------------------------------------------------------------------------
# Legacy simulation helpers (3-body, Pluto) — ported from backend/sim.py
# ---------------------------------------------------------------------------
...
def _grav_accel(body, bodies, G, softening=0.0):
    acc = np.zeros(2)
    for other in bodies:
        if other is not body:
            ...
            r = np.sqrt(r2)
            if r > 1e-10:
                acc += G * other.mass / (r2 * r) * r_vec
    return acc


def _simulate(bodies, dt, steps, G=None, softening=0.0, record_every=1):
    if G is None:
        G = G_SI
    history = []
    accels = [_grav_accel(b, bodies, G, softening) for b in bodies]
    for step in range(steps):
        if step % record_every == 0:
            history.append({
                "time": step * dt,
                "bodies": [{"name": b.name, "position": b.position.tolist(),
                            "velocity": b.velocity.tolist(), "mass": b.mass} for b in bodies],
            })
        for i, b in enumerate(bodies):
            b.position += b.velocity * dt + 0.5 * accels[i] * dt ** 2
        new_accels = [_grav_accel(b, bodies, G, softening) for b in bodies]
        for i, b in enumerate(bodies):
            b.velocity += 0.5 * (accels[i] + new_accels[i]) * dt
        accels = new_accels
    return history


def _create_three_body(positions=None, velocities=None, masses=None):
    n = 3
    if masses is None:
        masses = [3.0, 4.0, 5.0]
    ...
    com_v = np.zeros(2)
    for i in range(n):
        com_p += masses[i] * np.array(positions[i])
        com_v += masses[i] * np.array(velocities[i])
    com_p /= tm
    com_v /= tm
    positions = [list(np.array(positions[i]) - com_p) for i in range(n)]
    velocities = [list(np.array(velocities[i]) - com_v) for i in range(n)]
    return [_Body(masses[i], positions[i], velocities[i], f"Body {i+1}") for i in range(n)]


def _create_pluto_system():
    G = G_SI
    M_pluto, M_charon = 1.303e22, 1.586e21
    M_nix, M_styx, M_kerberos, M_hydra = 4.5e16, 7.5e15, 1.65e16, 4.8e16
    M_total = M_pluto + M_charon
    d_pc = 1.9571e7
    r_pluto = M_charon * d_pc / M_total
    r_charon = M_pluto * d_pc / M_total
    v_orb = math.sqrt(G * M_total / d_pc)
    v_pluto = M_charon / M_total * v_orb
    v_charon = M_pluto / M_total * v_orb
    moons = [
        (M_styx, 4.2656e7, 45, "Styx"),
        (M_nix, 4.8694e7, 135, "Nix"),
        (M_kerberos, 5.7783e7, 225, "Kerberos"),
        (M_hydra, 6.4738e7, 315, "Hydra"),
    ]
    bodies = [
        _Body(M_pluto, [-r_pluto, 0], [0, -v_pluto], "Pluto"),
        _Body(M_charon, [r_charon, 0], [0, v_charon], "Charon"),
    ]
    for mass, r, deg, name in moons:
        a = math.radians(deg)
        v = math.sqrt(G * M_total / r)
        bodies.append(_Body(mass, [r * math.cos(a), r * math.sin(a)],
                            [-v * math.sin(a), v * math.cos(a)], name))
    return bodies
...
def _handle_simulate(scenario: str, qs: dict) -> dict:
    ...
    try:
        if scenario == "three_body":
            pos = _parse_vecs(qs["pos"][0], 3) if "pos" in qs else None
            ...
            return {"error": "Unknown scenario", "scenario": scenario}
...
class SimServerHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # --- Legacy simulation API (3-body, Pluto) ---
        if path.startswith("/api/simulate/"):
            scenario = path.replace("/api/simulate/", "").strip("/")
            result = _handle_simulate(scenario, qs)
            self.send_json(result)
            return

        # --- Serve frontend/ files (legacy 3-body / Pluto viewer) ---
